...
class StatusInvestScraper:
    ...
    def _parse_demonstrativo(self, html: str, ticker: str, tipo: str) -> Demonstrativo:
        ...
        # O Status Invest usa classes específicas que mudam, então buscamos por padrão
        ...
        # Extrai headers (trimestres) - geralmente primeira linha
        headers = []
        header_row = table.find("thead")
        if header_row:
            ths = header_row.find_all("th")
            headers = [th.get_text(strip=True) for th in ths[1:]]  # Ignora coluna 'Conta'
        ...
        for row in rows:
            ...
            for col in cols[1:]:
                ...
                # Ex: "1.234,56" -> 1234.56, "1,23 Bi" -> 1230000000
        ...
        # Detecta unidade de medida (olhando para os valores ou legenda)
    ...
    def _detectar_unidade(self, html: str, contas: Dict) -> str:
        ...
        # Fallback: analisa magnitude dos valores
        amostra = []
    ...
    def save_to_csv(self, resultados: Dict[str, Demonstrativo], ticker: str):
        ...
        for tipo, demo in resultados.items():
            ...
            # Reorganiza colunas
            cols = ['ticker', 'tipo', 'unidade', 'data_extracao'] + [c for c in df.columns if c not in ['ticker', 'tipo', 'unidade', 'data_extracao']]
            df = df[cols]
...
async def main():
    ...
    scraper = StatusInvestScraper(headless=True)  # headless=False para debug visual
    ...
    for ticker in tickers:
        ...
        # Delay entre tickers para não sobrecarregar
